...
describe 'Atlassian Confluence with Embedded Database' do
	...
	include_examples 'a buildable Docker image', '.', { env: ["CATALINA_OPTS=-Xms3g -Xmx3g -XX:+UseG1GC -XX:+PerfDisableSharedMem -XX:+ParallelRefProcEnabled -XX:G1HeapRegionSize=8m -XX:MaxGCPauseMillis=250 -XX:InitiatingHeapOccupancyPercent=75 -XX:+UseLargePages -XX:+AggressiveOpts -Datlassian.plugins.enable.wait=#{Docker::DSL.timeout}"] }
...
describe 'Atlassian Confluence with PostgreSQL 9.3 Database' do
    ...
    after :all do
			if ENV['CIRCLECI']
				@container_db.kill signal: 'SIGKILL'
			else
      	@container_db.remove force: true, v: true
			end
    end
  end
end
...
describe 'Atlassian Confluence behind reverse proxy' do
	...
	include_examples 'a buildable Docker image', '.', {
		env: [
			"CATALINA_OPTS=-Xms3g -Xmx3g -XX:+UseG1GC -XX:+PerfDisableSharedMem -XX:+ParallelRefProcEnabled -XX:G1HeapRegionSize=8m -XX:MaxGCPauseMillis=250 -XX:InitiatingHeapOccupancyPercent=75 -XX:+UseLargePages -XX:+AggressiveOpts -Datlassian.plugins.enable.wait=#{Docker::DSL.timeout}",
	...
	include_examples 'a minimal acceptable Confluence instance' do
		...
		after :all do
			if ENV['CIRCLECI']
				@container_proxy.kill signal: 'SIGKILL'
			else
				@container_proxy.remove force: true, v: true
			end
		end
	end
