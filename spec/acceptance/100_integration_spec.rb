describe 'Atlassian Confluence with Embedded Database', retry: 3 do
...
describe 'Atlassian Confluence with PostgreSQL 9.3 Database', retry: 3 do
  include_examples 'a buildable Docker image', '.', env: ["CATALINA_OPTS=-Xms1024m -Xmx1024m -XX:+UseG1GC -Datlassian.plugins.enable.wait=#{Docker::DSL.timeout}"]
...
describe 'Atlassian Confluence with MySQL 5.6 Database', retry: 3 do
  include_examples 'a buildable Docker image', '.', env: ["CATALINA_OPTS=-Xms1024m -Xmx1024m -XX:+UseG1GC -Datlassian.plugins.enable.wait=#{Docker::DSL.timeout}"]
...
describe 'Atlassian Confluence behind reverse proxy', retry: 3 do
