...
describe "Atlassian Confluence acceptance" do
  ...
  let(:regex_filter)  { /To prevent a memory leak, the JDBC Driver has been forcibly unregistered/   }
  ...
  context "when Atlassian Confluence is shut down" do
    ...
    it "has shut down" do
      ...
      # SIGTERM
      expect($container.wait 300).to including("StatusCode" => 143)
    end

    it "has no severe in the stdout" do
      expect((scan_stdout regex_severe).select{|v| v !~ regex_filter}).to be_empty
    end
