...
RUN wget "http://www.atlassian.com/software/confluence/downloads/binary/atlassian-confluence-${CONFLUENCE_VERSION}.tar.gz" \
    ...
    && chown -R confluence:confluence "/usr/local/atlassian/confluence/temp" \
    && chown -R confluence:confluence "/usr/local/atlassian/confluence/logs" \
    && chown -R confluence:confluence "/usr/local/atlassian/confluence/work" \
